...
def final_multiplication(inputs, weights):
	...
	for y in range(0,d):
		...
		for l in range(0,8):
			...
			sum_f= int((sum_f/25)) #(removed *2 from here)
			...
			if length==4:
				...
				for i in range(0,4-len(binary_f)):
		...
		if len(answer)==12:
			...
			for h in range(0,(12-len(answer))):
				answer='0'+answer
...
def actual_convolution(x,y):
	...
	for i in range(0,len(actual_conv)):
		...
		convolve= bin(actual_conv[i]//25)
		convolve= str(convolve[2:: ])

		if len(convolve)==12:
			convolve= convolve

		elif len(convolve)>12:
			convolve= convolve[:12]

		elif len(convolve)<12:
			for u in range(0,(12-len(convolve))):
				convolve='0'+convolve
